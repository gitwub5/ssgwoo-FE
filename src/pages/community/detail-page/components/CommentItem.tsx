...
export function CommentItem({
  ...
  const handleFinishEdit = async () => {
    try {
      ...
      if (success) {
        onFinishEdit()
        alert(t('communityPage.detailPage.comment.editSuccess'))
      }
    } catch (error) {
      console.error('댓글 수정 실패:', error)
      alert(t('communityPage.detailPage.comment.error.editFailed'))
    }
  }
        ...
        <div className="ml-auto relative">
          ...
          {openMenu && (
            <div className="absolute right-0 mt-2 w-24 bg-white border rounded shadow z-10">
              <button onClick={() => { setOpenMenu(false); onEdit(); }} className="block w-full px-4 py-2 text-left hover:bg-gray-100">
                {t('communityPage.detailPage.comment.edit')}
              </button>
              <button onClick={() => { setOpenMenu(false); onDelete(); }} className="block w-full px-4 py-2 text-left text-red-500 hover:bg-gray-100">
                {t('communityPage.detailPage.comment.delete')}
              </button>
            </div>
          )}
        </div>
